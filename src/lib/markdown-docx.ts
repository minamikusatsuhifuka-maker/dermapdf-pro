...
// 出力するユーティリティ。PDF出力（画像化）とは別で、Wordで通常に編集できるテキスト＋スタイル。
// 依存: docx（純JS・クライアント生成）。日本語フォントは游明朝を明示指定する。
...
// PDF/MD出力と同じヘッダ構成（タイトル＋メタ行）を踏襲しつつ、本文は編集可能な実体スタイル。
export async function exportMarkdownAsDocx(opts: {
  title: string;
  metaLines: string[];
  markdown: string;
  fileName: string;
