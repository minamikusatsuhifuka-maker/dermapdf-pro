...
export function bulkExportAsSingleMarkdown(records: AnalysisRecord[]): void {
...
`;
  ...
  const md = `# DermaPDF Pro 一括エクスポート（${records.length}件）
...
`;
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `dermapdf_bulk_${records.length}件_${dateFileStr}.md`;
  a.click();
  URL.revokeObjectURL(url);
}

export function bulkExportAsText(recs: AnalysisRecord[]): void {
  const dateFileStr = new Date().toISOString().split("T")[0];
...
`;
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `dermapdf_bulk_${recs.length}件_${dateFileStr}.txt`;
  a.click();
  URL.revokeObjectURL(url);
}
