...
export function AnalysisStockPanel() {
  ...
  // 本文の閲覧⇄編集の切替（null=閲覧＝Markdownレンダリング／id=そのカードを編集中＝生テキストcontentEditable）。
  ...
  // 本文の編集を終了して閲覧（レンダリング）表示に戻す。
  // 終了時に DOM の最新 innerHTML を既存の保存経路（updateAnalysisContent＋同一の空ガード）で
  // 確定保存してから reload。debounce 待ちの取りこぼしを防ぎ、閲覧表示に最新内容を反映する。
  // ※ 保存関数・ガードは既存のものをそのまま使用（保存ロジックは変更しない）。
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 px-4 py-3 space-y-2">
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] text-gray-400">
                      ...
                      {/* 閲覧⇄編集トグル: 閲覧時はMarkdownレンダリング、編集時は生テキストで編集 */}
                      {bodyEditingId === r.id ? (
                        <button
                    ...
                    <div>
                      {bodyEditingId === r.id ? (
                        ...
                        /* 閲覧モード: 画面表示と同じ Markdown レンダリング（読み取り専用）。
                           react-markdown(+rehype-raw)なので生Markdownの見出し等もHTML保存後の内容も同じ見た目で表示。 */
                        <div
                          onClick={(e) => e.stopPropagation()}
