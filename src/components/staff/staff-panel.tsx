...
function StaffForm({
  ...
  return (
    <div className="space-y-3 rounded-xl border border-gray-200 bg-white p-4">
      ...
      <button onClick={handleSubmit} className="w-full rounded-lg bg-gradient-to-r from-rose-400 to-purple-400 px-4 py-2 text-sm font-bold text-white">
        {initial ? "更新" : "追加"}
      </button>
    </div>
  );
          ...
          {staff.dominantNeeds.length > 0 && (
            <div className="mt-1 flex flex-wrap gap-1">
              {staff.dominantNeeds.map((n) => (
                <span key={n} className="rounded-full bg-purple-100 px-2 py-0.5 text-[10px] font-medium text-purple-700">
          ...
          <div className="space-y-3">
            ...
            {showAddRecord && (
              <div className="space-y-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
                ...
                <button onClick={handleAddRecord} className="w-full rounded-lg bg-purple-500 px-3 py-2 text-xs font-bold text-white">
                  保存
                </button>
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-2 border-t pt-3">
              <button
                ...
                disabled={aiLoading}
                className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-blue-500 to-purple-500 px-3 py-2 text-xs font-bold text-white disabled:opacity-40"
              >
              ...
              <button
                ...
                disabled={aiLoading}
                className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-pink-500 to-rose-500 px-3 py-2 text-xs font-bold text-white disabled:opacity-40"
              >
                {aiLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : null}
                目標応援メッセージを生成
              </button>
            ...
            {aiResult && (
              <div className="space-y-2 rounded-xl border border-purple-100 bg-purple-50/30 p-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleSaveAiToStock} className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-rose-400 to-purple-400 px-3 py-1.5 text-xs font-medium text-white shadow-sm">
            ...
            <div>
              ...
              {staff.dominantNeeds.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {staff.dominantNeeds.map((n) => (
                    <span key={n} className="rounded-full bg-purple-100 px-3 py-1 text-xs font-medium text-purple-700">
                      {NEEDS_OPTIONS.find((o) => o.value === n)?.label || n}
                    </span>
                  ))}
                </div>
          ...
          {profiles.map((p) => (
            <div key={p.id} className="rounded-xl border border-gray-100 bg-white/60 p-4 space-y-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-blue-500 to-purple-500 px-3 py-1.5 text-xs font-bold text-white"
              >
                カルテを開く
              </button>
            </div>
