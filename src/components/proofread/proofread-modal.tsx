...
${numbered}`;
      // 項目過多でも出力が切れないよう枠を確保
      const data = await analyzeTextWithGemini(prompt, undefined, 32768);
      ...
      const endPos = cleaned.lastIndexOf("]");
      if (!data.success || start === -1 || endPos === -1) {
        setError(
          data.error || "校正候補の解析に失敗しました。再度お試しください。"
        );
        setIssues([]);
        setLoading(false);
        return;
      }
      const arr = JSON.parse(cleaned.slice(start, endPos + 1)) as RawIssue[];
      setIssues(
        arr.map((it, i) => ({
          id: i,
